...
function LevelMaker.createMap(level)
    ...
    local numRows = math.random(4, 8)

    -- randomly choose the number of columns, ensuring odd
    local numCols = math.random(9, 13)
    ...
    --[[if numRows>=3 then
        local m = math.random(#bricks)
        local x = bricks[m].x
        local y = bricks[m].y
        lockbrick = Brick(x,y)
        lockbrick.locked = true
        table.remove(bricks,m)
        table.insert(bricks,lockbrick)]]
        --[[for i = #bricks, m+1, -1 do
          local a = bricks[i]
          bricks[i] = bricks[i-1]
          bricks[i-1] = a
        end]]
